...
public class ReplicationGroupManager implements ReplicationGroupManagerMBean {
    ...
    public synchronized void registerJmx() throws SQLException {
        if (!this.isJmxRegistered) {
            try {
                ...
                throw SQLError.createSQLException("Unable to register replication host management bean with JMX", (String) null, (Throwable) e, (ExceptionInterceptor) null);
            }
        }
    }
    ...
    public String getRegisteredConnectionGroups() {
        StringBuilder sb = new StringBuilder("");
        boolean found = false;
        for (ReplicationConnectionGroup group : ReplicationConnectionGroupManager.getGroupsMatching((String) null)) {
            if (found) {
                sb.append(",");
            }
            found = true;
