...
public class XmlRpcClient {
    ...
    public Set<String> getGroupNames() throws FabricCommunicationException {
        ...
        for (Map<String, ?> row : errorSafeCallMethod(METHOD_GROUP_LOOKUP_GROUPS, (Object[]) null).getResultSet()) {
    ...
    public FabricStateResponse<Set<ServerGroup>> getServerGroups(String groupPattern) throws FabricCommunicationException {
        ...
        for (Map.Entry<String, Set<Server>> entry : serversByGroupName.entrySet()) {
            serverGroups.add(new ServerGroup(entry.getKey(), entry.getValue()));
        }
    ...
    private FabricStateResponse<Set<ShardTable>> getShardTables(int shardMappingId) throws FabricCommunicationException {
        Response tablesResponse = errorSafeCallMethod(METHOD_DUMP_SHARD_TABLES, new Object[]{0, String.valueOf(shardMappingId)});
    ...
    private FabricStateResponse<Set<ShardIndex>> getShardIndices(int shardMappingId) throws FabricCommunicationException {
        Response indexResponse = errorSafeCallMethod(METHOD_DUMP_SHARD_INDEX, new Object[]{0, String.valueOf(shardMappingId)});
        Set<ShardIndex> indices = new HashSet<>();
        for (Map<String, ?> rawIndexEntry : indexResponse.getResultSet()) {
            indices.add(new ShardIndex((String) rawIndexEntry.get(FIELD_LOWER_BOUND), Integer.valueOf(((Integer) rawIndexEntry.get(FIELD_SHARD_ID)).intValue()), (String) rawIndexEntry.get(FIELD_GROUP_ID)));
        }
    ...
    public FabricStateResponse<Set<ShardMapping>> getShardMappings(String shardMappingIdPattern) throws FabricCommunicationException {
        Response mapsResponse = errorSafeCallMethod(METHOD_DUMP_SHARD_MAPS, new Object[]{0, shardMappingIdPattern});
        ...
        for (Map<String, ?> rawMapping : mapsResponse.getResultSet()) {
            ...
            mappings.add(new ShardMappingFactory().createShardMapping(mappingId, shardingType, globalGroupName, tables.getData(), indices.getData()));
        }
    ...
    public int createShardMapping(ShardingType type, String globalGroupName) throws FabricCommunicationException {
        return ((Integer) errorSafeCallMethod(METHOD_SHARDING_CREATE_DEFINITION, new Object[]{type.toString(), globalGroupName}).getResultSet().get(0).get(FIELD_RESULT)).intValue();
    }
